...
export const actions = pairs.map(({ id, action, reaction }) => ({
    id,
    action,
...
export const reactions = pairs.map(({ id, action, reaction }) => ({
    id,
    actionSource: action,
    reaction,
    status: 'paired'
...
export const findAction = (id) => actions.find((item) => item.id === id);
export const findReaction = (id) => reactions.find((item) => item.id === id);

export const TOTAL_APIS = 21;
